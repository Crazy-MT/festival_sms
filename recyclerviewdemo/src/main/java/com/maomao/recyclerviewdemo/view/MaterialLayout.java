package com.maomao.recyclerviewdemo.view;
...
import com.maomao.recyclerviewdemo.R;


/**
 * Created by Mao on 2016/2/6.
 */
public class MaterialLayout extends LinearLayout {
    ...
    private void initTypedArray(Context context, AttributeSet attrs) {
        ...
        mCirclelColor = typedArray.getColor(R.styleable.MaterialLayout_color_material_layout, Color.LTGRAY);
        mDuration = typedArray.getInteger(R.styleable.MaterialLayout_duration_material_layout,
                DEFAULT_DURATION);
        mFrameRate = typedArray
                .getInteger(R.styleable.MaterialLayout_frame_rate_material_layout, DEFAULT_FRAME_RATE);
        mColorAlpha = typedArray.getInteger(R.styleable.MaterialLayout_alpha_material_layout, DEFAULT_ALPHA);
        mCircleScale = typedArray.getFloat(R.styleable.MaterialLayout_scale_material_layout, DEFAULT_SCALE);
     ...
     * 减去状态栏和标题栏的高度
     */
    private void removeExtraHeight(MotionEvent event) {
        int[] location = new int[2];
        ...
        // 获取中心点
        mCenterPoint = new Point((int) event.getX(), (int) event.getY());
    ...
    private void calculateMaxRadius(View view) {
        ...
        //取对角线长度
        mMaxRadius = (int) Math.sqrt(view.getWidth() * view.getWidth() + view.getHeight() * view.getHeight());
    ...
    private void drawRippleIfNecessary(Canvas canvas) {
        if (isFoundTouchedSubView()) {
            ...
            //绘制背景灰色
            canvas.drawColor(Color.argb(100,96, 96, 96));
            // 绘制背景圆形,也就是
